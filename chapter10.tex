...
各マスが黒（1）か白（0）である$n \times n$個のグリッドがあるとき、すべての角が黒であるサブグリッドの数を計算する問題を考えます。
...
例えば，上のシナリオでは price[2][3] = 7 である． この関数を用いると，問題の解は total({0 ... k - 1}, n - 1) となる．
まず、空集合を買うにはコストがかからないのでtotal((/), d) = 0、初日に商品を1つ買う方法は1つなのでtotal({x}, 0) = price[x][0] である。そうすると、次のような漸化式を用いることができる。

ここで、$\texttt{price}[x][d]$ は$d$での$x$の価格を示すとしましょう。$x$
...
次に、ダイナミックプログラミングにより、関数の値を計算する。関数の値を格納するための配列を用意しましょう。
\begin{lstlisting}
int total[1<<K][N];
\end{lstlisting}
