...
ノード1からノードxへの経路の数をpaths(x)とすると、基本ケースとして paths(1)=1である。
そして、paths(x)の他の値を計算するために、再帰的に

ノード1からあるノード$x$へのパスの経路数を$\texttt{paths}(x)$と表現するとします。
$\texttt{paths}(1)=1$です。
...
例えば、paths(3)の値を計算する場合、ノード2とノード5からノード3への辺 があるので、
paths(2) + paths(5)という式を用いることができる。paths(2) = 2、 paths(5) = 1なので、paths(3) = 3と結論付けられる。
...
動的計画問題の問題というのは有向無サイクルグラフで表現することができます。
...
サクセスグラフは1つ以上のコンポートんとからなます。
それぞれのコンポーネントには1つのサイクルとそれにつながるパスがあります。
サクセスグラフはfunctionalグラフと呼ばれることもある。
...
\key{フロイドの循環検出アルゴリズム - Floyd's algorithm}
